...
import { ShoppingBag, Play, Image as ImageIcon, DollarSign, ArrowLeft, Briefcase, Shield, Trash2, X, Download, Lock, Check, Zap } from 'lucide-react';
        ...
        {!user.isStoreActive && !isOwnStore ? (
          ...
          <>
            ...
            {(activeSection === 'all' || activeSection === 'videos') && (
              <div className="mb-16 space-y-16 animate-in fade-in duration-500">
                
                {/* $20 Videos Subsection */}
                <div className="space-y-6">
                  <div className="flex items-center space-x-4 mb-4">
                    ...
                    <div>
                      <h3 className="text-xl font-black uppercase tracking-tight" style={{ color: customization.fontColor }}>$20 Videos</h3>
                      <p className="text-[10px] uppercase font-black tracking-widest text-[#967bb6] mt-0.5">Verified 2 to 10 Minutes Tier • 80% Payout Split</p>
                    </div>
                    <div className="flex-grow h-px bg-white/5"></div>
                  </div>
                  
                  {videos.filter(v => (v.videoDuration || 0) >= 120 && (v.videoDuration || 0) <= 600).length > 0 ? (
                    <div className={`grid gap-8 ${
                      ...
                      {videos.filter(v => (v.videoDuration || 0) >= 120 && (v.videoDuration || 0) <= 600).map((item, index) => (
                    ...
                    <div className="glass-panel rounded-[2rem] p-10 text-center border border-dashed border-white/5">
                      <p className="text-slate-500 font-black uppercase tracking-widest text-[9px] mb-1">No videos listed in this tier</p>
                      <p className="text-slate-600 text-[8px] uppercase tracking-wider">Videos between 2 and 10 minutes appear here.</p>
                    </div>
                  )}
                </div>

                {/* $40 Videos Subsection */}
                <div className="space-y-6">
                  <div className="flex items-center space-x-4 mb-4">
                    <div className="p-3 rounded-2xl" style={{ backgroundColor: `${customization.accentColor}20`, color: customization.accentColor }}>
                      <Zap size={20} />
                    ...
                    <div>
                      <h3 className="text-xl font-black uppercase tracking-tight" style={{ color: customization.fontColor }}>$40 Videos</h3>
                      <p className="text-[10px] uppercase font-black tracking-widest text-[#967bb6] mt-0.5">Ultra Premium Over 10 Minutes Cinema Tier • 80% Payout Split</p>
                    </div>
                    <div className="flex-grow h-px bg-white/5"></div>
                  </div>
                  
                  {videos.filter(v => (v.videoDuration || 0) > 600).length > 0 ? (
                    <div className={`grid gap-8 ${
                      customization.layout === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' :
                      customization.layout === 'list' ? 'grid-cols-1' :
                      'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'
                    }`}>
                      {videos.filter(v => (v.videoDuration || 0) > 600).map((item, index) => (
                        <React.Fragment key={item.id}>
                          <StoreCard 
                            item={item} 
                            isAdmin={isAdmin} 
                    ...
                    <div className="glass-panel rounded-[2rem] p-10 text-center border border-dashed border-white/5">
                      <p className="text-slate-500 font-black uppercase tracking-widest text-[9px] mb-1">No cinema-length videos in this tier</p>
                      <p className="text-slate-600 text-[8px] uppercase tracking-wider">Premium videos longer than 10 minutes appear here.</p>
                    </div>
                  )}
                </div>

                {/* Short Clips & Free Teasers Subsection */}
                {videos.filter(v => (v.videoDuration || 0) < 120).length > 0 && (
                  <div className="space-y-6">
                    <div className="flex items-center space-x-4 mb-4">
                      <div className="p-3 rounded-2xl" style={{ backgroundColor: `${customization.accentColor}20`, color: customization.accentColor }}>
                        <Play size={20} />
                      ...
                      <div>
                        <h3 className="text-xl font-black uppercase tracking-tight" style={{ color: customization.fontColor }}>Free Teasers & Promos</h3>
                        <p className="text-[10px] uppercase font-black tracking-widest text-slate-500 mt-0.5">Clips Under 2 minutes • Playable instantly</p>
                      </div>
                      <div className="flex-grow h-px bg-white/5"></div>
                    </div>
                    ...
                    <div className={`grid gap-8 ${
                      ...
                      {videos.filter(v => (v.videoDuration || 0) < 120).map((item, index) => (
                        <React.Fragment key={item.id}>
                          <StoreCard 
                            item={item} 
                            isAdmin={isAdmin} 
                            isOwnItem={isOwnStore}
                            isPurchased={true} // Free teasers allow immediate play
                            customization={customization} 
                            onDelete={(isOwnStore || isAdmin) ? () => onDeleteItem?.(item.id) : undefined} 
                            onBuy={() => {}}
                          />
...
const StoreCard: React.FC<{ item: StoreItem; isAdmin?: boolean; isOwnItem?: boolean; isPurchased?: boolean; customization: any; onDelete?: () => void; onBuy?: () => void }> = ({ item, isAdmin, isOwnItem, isPurchased, customization, onDelete, onBuy }) => {
  ...
  // Pricing detection based on rules if item.price is absent
  ...
  return (
    <div className={`group ${customization.layout === 'list' ? 'flex items-center space-x-8 p-6 rounded-[2.5rem] border border-white/5 bg-white/[0.02]' : ''}`}>
      ...
      >
        {/* Blurry locked image card or play item */}
        {finalIsVideo && canView ? (
          ...
          <div className="w-full h-full relative">
            <img 
              ...
              alt={item.title}
              className={`w-full h-full object-cover transition-transform duration-700 ${!canView ? 'blur-md opacity-60 scale-[1.03]' : 'group-hover:scale-110'}`}
              onError={(e) => {
                const target = e.target as HTMLImageElement;
                if (target.src !== APP_LOGO_URL) {
                  target.src = APP_LOGO_URL;
        ...
        {/* Lock Overlay */}
        {!canView && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-4 z-10 text-center space-y-3">
            <div className="w-12 h-12 rounded-2xl bg-black/60 border border-white/20 backdrop-blur-md flex items-center justify-center shadow-lg animate-pulse">
              <Lock size={20} className="text-amber-500" />
            </div>
            <div>
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#967bb6] bg-black/60 px-2.5 py-1 rounded">
                {item.type === 'video' ? 'Locked Video' : item.type === 'picture_pack' ? 'Locked Picture Pack' : 'Locked Media'}
              </span>
              {displayPrice && (
                <div className="text-lg font-black text-white mt-1 uppercase tracking-tight scale-105 bg-emerald-500/90 px-3 py-1 rounded-xl shadow-lg border border-emerald-400/20 inline-block font-mono">
        ...
        <div className="absolute top-4 right-4 flex flex-col space-y-2 items-end z-20">
          {canView && (
        ...
        {finalIsVideo && canView && (
          <div 
            ...
            className="absolute bottom-4 right-4 w-12 h-12 rounded-full flex items-center justify-center shadow-2xl cursor-pointer hover:scale-110 z-20" 
            style={{ backgroundColor: customization.accentColor, boxShadow: `0 0 20px ${customization.accentColor}60` }}
          >
      ...
      <div className={`px-2 ${customization.layout === 'list' ? 'flex-grow' : ''}`}>
        ...
        {/* Bottom Lock / Buy status or content type */}
        <div className="mt-2 flex items-center justify-between gap-4">
          <p className="text-[9px] font-black uppercase tracking-widest" style={{ color: customization.accentColor }}>
          ...
          {item.type === 'video' && item.videoDuration && (
            <span className="text-[9px] text-slate-500 font-mono font-bold">
              {Math.floor(item.videoDuration / 60)}m {item.videoDuration % 60}s
            </span>
          )}
        </div>
        ...
        {!canView && onBuy && (
          <button 
            ...
            className="w-full mt-4 bg-emerald-500 hover:bg-emerald-600 text-white font-black text-[10px] uppercase tracking-[0.15em] py-4 rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg shadow-emerald-500/10"
          >
            <Lock size={12} />
            <span>
              Unlock {item.type === 'video' ? 'Video' : item.type === 'picture_pack' ? 'Pack' : 'Media'} ${displayPrice?.toFixed(2) || '15.00'}
            </span>
          </button>
        )}
      </div>
